...
@Aspect
@Component
public class WebsiteLoginVerificationAspect {

    @Resource
    private RedisTemplate redisTemplate;

    @Pointcut("@annotation(com.ilhaha.yueyishou.common.anno.WebsiteLoginVerification)")
    ...
    public void wechatLoginVerification(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        String operatorId = (String) redisTemplate.opsForValue().get(RedisConstant.OPERATE_LOGIN_KEY_PREFIX + token);
        if (ObjectUtils.isEmpty(operatorId)) {
            AuthContextHolder.removeOperatorId();
            throw new YueYiShouException(ResultCodeEnum.LOGIN_AUTH);
        }
        if (!ObjectUtils.isEmpty(operatorId)) {
            AuthContextHolder.setOperatorId(Long.valueOf(operatorId));
        }
